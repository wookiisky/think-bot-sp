...
11. 页面级清空后，若用户直接点击某个快捷 `promptTab` 且页面正文为空，side panel 以 `prompt_tab_click` 来源发起 Readability 提取；成功后只继续发送当前点击的快捷 `promptTab`。
12. 全部失败则返回错误态，并保留当前页面上下文。
...
- `prompt_tab_click` 兜底提取固定使用 Readability，不沿用当前页面方法。
...
- 页面级清空后手动点击某个快捷 `promptTab` 只触发当前标签；缺正文时先补一次 Readability 提取，不执行全量后台自动触发。
...
- 页面级清空后，重新进入页面可以重新提取并重新自动触发；若不重开页面而直接点击某个快捷 `promptTab`，则先用 Readability 补提取再发送当前标签。
