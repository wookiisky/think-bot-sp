...
- 90 天未更新则可被清理。
- 标签首次手动发送或自动触发开始时写入对应 `tabStates.initializedAt`。
- 清空当前标签时重置该标签的 `initializedAt / lastAutoTriggerAt / autoTriggerStatus`，并记录 `lastClearedAt`。
- 风险：
  - URL 归一化不一致会导致重复页面。
  - 提取失败覆盖旧内容会导致历史丢失。
  - 标签初始化状态未持久化会导致 side panel 重开后重复自动触发。
