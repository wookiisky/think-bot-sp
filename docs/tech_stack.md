...
- 本地 WXT dev server 固定绑定 `127.0.0.1:3000`，避免 Windows 环境下 `localhost` 解析到 IPv6 回环地址后，扩展页连接 `ws://localhost:3000/` 被拒绝。
...
- 运行时结构化 `console`
  - 用于记录关键流程 debug 日志。
  - 每条日志是一行文本 `[scope] event {json}`，载荷在记录时刻序列化。
  - 级别阈值：开发与测试默认 `debug`，生产默认 `info`；在对应 console 调用 `__thinkBotLog.setLevel('debug')` 可临时放开。
