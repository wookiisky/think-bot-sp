...
- 正常流：
  - 左侧导航固定为 5 个栏目，顶部动作区固定为“保存 / 保存并同步 / 恢复默认 / 导入配置 / 导出配置”。
  ...
  - 快捷输入支持编辑名称、提示词、自动触发、专属模型、分支模型、折叠展开和消息预览；专属模型与分支模型同排展示。
  ...
  - 基础设置和快捷输入中的分支模型选择统一使用多选下拉。
  - 基础设置支持配置默认提取区高度、Jina API Key 和 Jina 响应模板。
  - 侧边栏会读取默认提取区高度，Jina 提取会真实消费 API Key 与响应模板。
