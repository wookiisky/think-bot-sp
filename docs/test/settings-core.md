...
- 正常流：
  ...
  - 展示配置支持分别设置 `h1 / h2 / h3 / h4 / body / list` 的字号、颜色和下划线，以及 `strong`（粗体）的颜色。
  - 展示配置顶部 8 个内置配色按钮（蓝色、橙色、青色、石墨、暖阳、极光、森林、莓果）能立即回写草稿，并在保存后持久化；点击后粗体颜色同步为该配色的配套粗体色（蓝色为纯黑）。
  - 助手 Markdown 里的列表项按 `list` 样式渲染，正文和列表项里的粗体按 `strong.color` 着色，标题里的粗体保持标题色。
...
- 异常流：
  - 快速重复保存。
  - 同步期间切换栏目仍禁止修改配置；同步成功或失败后恢复编辑。
  - 远端模板导入期间允许编辑草稿，返回模板追加到最新草稿；期间禁止保存、重置、导入配置和同步，避免全量配置回写覆盖追加结果。
  - 基础设置草稿切换栏目后不丢失。
