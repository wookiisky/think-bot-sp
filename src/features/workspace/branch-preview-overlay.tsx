...
import { WorkspaceStatusGlyph, type WorkspaceVisualStatus } from './workspace-status';
...
/** 把分支状态映射成统一视觉状态。 */
const toPreviewVisualStatus = (status: BranchPreviewDetail['status']): WorkspaceVisualStatus => {
  switch (status) {
    case 'loading':
      return 'loading';
    case 'error':
...
/** 解析分支状态文案。 */
const resolvePreviewStatusLabel = (status: BranchPreviewDetail['status'], t: WorkspaceTranslator) => {
  switch (status) {
    case 'loading':
...
export const BranchPreviewOverlay = ({ open, preview, t, onClose }: BranchPreviewOverlayProps) => {
  ...
  const statusLabel = useMemo(
    () => (preview ? resolvePreviewStatusLabel(preview.status, t) : ''),
    [preview, t],
  );
          ...
          <div className="min-w-0 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <h2 id={titleId} className="text-base font-semibold text-foreground">
                {t('workspace.branchPreviewTitle')}
              </h2>
              <Badge variant="outline">{t(preview.isPrimary ? 'workspace.status.primaryBranch' : 'workspace.status.branch')}</Badge>
              <Badge variant="secondary">{preview.modelLabel}</Badge>
              <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                <WorkspaceStatusGlyph label={statusLabel} status={toPreviewVisualStatus(preview.status)} className="size-3.5" />
                {statusLabel}
              </span>
            </div>
            <p id={descriptionId} className="text-sm text-muted-foreground">
              {t('workspace.branchPreviewDescription')}
            </p>
          </div>
