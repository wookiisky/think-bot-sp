...
test('side panel 可以发送消息、收到首包流式并在完成后写入历史', async ({ context, extensionId }) => {
  ...
  await sidepanel.getByLabel('聊天输入').press('Enter');
...
test('side panel 支持 quickInputs 多标签切换，并隔离草稿与会话', async ({ context, extensionId }) => {
  ...
  await expect(sidepanel.getByLabel('聊天输入')).toHaveValue('');
  ...
  await expect(sidepanel.getByRole('tabpanel', { name: /翻译/ }).getByText('翻译')).toBeVisible();
  await expect(sidepanel.getByLabel('聊天输入')).toHaveValue('');
...
test('页面首次提取成功后会自动触发 quickInput，且重开 side panel 不重复触发', async ({ context, extensionId }) => {
  ...
  await expect(reopened.getByRole('tab', { name: /总结/ })).toHaveAttribute('title', /自动触发完成/);
...
test('标签级清空只移除当前 promptTab 历史，保留页面正文和其他标签会话', async ({ context, extensionId }) => {
  ...
  await sidepanel.getByRole('button', { name: '发送' }).click();
  await expect(sidepanel.getByRole('tabpanel', { name: /总结/ }).getByText('标签清空测试响应').last()).toBeVisible();

  await sidepanel.getByRole('button', { name: '清空当前标签' }).click();
  await sidepanel.getByTestId('clear-tab-confirm').getByRole('button', { name: '清空当前标签' }).click();
...
test('图片预览可移除，提取区和输入区支持拖拽后仍可发送', async ({ context, extensionId }) => {
  ...
  await sidepanel.getByTestId('remove-image-confirm-1').getByRole('button', { name: '移除图片' }).click();
